...
import { ACTION_MATCH_ROUNDS, twemojiUrl } from "@/lib/game-data";
import { bumpGameModuleRound, recordGameAttempt } from "@/lib/game-progress";

export function ActionMatchGame({ token }: { token: string }) {
  const [progress, setProgress] = useState(0);
  return (
    <GameShell token={token} progress={progress} onRoundComplete={() => bumpGameModuleRound(token, "action-match")}>
      <ActionMatchInner token={token} onProgress={setProgress} />
    </GameShell>
  );
}

function ActionMatchInner({
  token,
  onProgress,
}: {
  token: string;
  ...
  const indices = useMemo(() => shuffle(ACTION_MATCH_ROUNDS.map((_, i) => i)), []);
  const [cursor, setCursor] = useState(0);
  const [busy, setBusy] = useState(false);
  const [correctInWave, setCorrectInWave] = useState(0);

  const round = ACTION_MATCH_ROUNDS[indices[cursor % indices.length]];
  ...
  function replay() {
    if (busy) return;
    void speakHebrew(`${round.place}. ${round.actionHint}`);
  }

  function handlePick(file: string) {
    if (busy) return;
    if (file !== round.correct) {
      recordGameAttempt(token, "action-match", "wrong", `«${round.place}» — בחירה לא נכונה`);
      playSoftBuzz();
      if (typeof navigator !== "undefined" && navigator.vibrate) navigator.vibrate(35);
      return;
    }
    setBusy(true);
    recordGameAttempt(token, "action-match", "correct");
    playCorrectChime();
    triggerStars();
  ...
  return (
    <div className="flex flex-col items-center gap-8 px-2">
      <div className="flex flex-col items-center gap-4">
        <p className="text-center text-2xl font-bold text-slate-800 dark:text-slate-100">מה עושים שם?</p>
        <button
          type="button"
          onClick={replay}
          disabled={busy}
      ...
      <div className="flex flex-col items-center gap-2 rounded-3xl border-4 border-orange-200 bg-orange-50/80 px-8 py-6 text-center dark:border-orange-700 dark:bg-orange-950/30">
        <p className="text-4xl font-bold">{round.place}</p>
        <p className="text-xl text-slate-700 dark:text-slate-200">{round.actionHint}</p>
      </div>
