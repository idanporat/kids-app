...
import { CONTEXT_MATCH_ROUNDS, twemojiUrl } from "@/lib/game-data";
import { bumpGameModuleRound, recordGameAttempt } from "@/lib/game-progress";

export function ContextMatchGame({ token }: { token: string }) {
  const [progress, setProgress] = useState(0);
  return (
    <GameShell token={token} progress={progress} onRoundComplete={() => bumpGameModuleRound(token, "context-match")}>
      <ContextMatchInner token={token} onProgress={setProgress} />
    </GameShell>
  );
}

function ContextMatchInner({
  token,
  onProgress,
}: {
  token: string;
  ...
  const indices = useMemo(() => shuffle(CONTEXT_MATCH_ROUNDS.map((_, i) => i)), []);
  const [cursor, setCursor] = useState(0);
  const [busy, setBusy] = useState(false);
  const [correctInWave, setCorrectInWave] = useState(0);

  const round = CONTEXT_MATCH_ROUNDS[indices[cursor % indices.length]];
  ...
  function replay() {
    if (busy) return;
    void speakHebrew(`מה שייך ל־${round.place}?`);
  }

  function handlePick(file: string) {
    if (busy) return;
    if (file !== round.correct) {
      recordGameAttempt(token, "context-match", "wrong", `מקום «${round.place}» — בחירה לא נכונה`);
      playSoftBuzz();
      if (typeof navigator !== "undefined" && navigator.vibrate) navigator.vibrate(35);
      return;
    }
    setBusy(true);
    recordGameAttempt(token, "context-match", "correct");
    playCorrectChime();
    triggerStars();
  ...
  return (
    <div className="flex flex-col items-center gap-8 px-2">
      <div className="flex flex-col items-center gap-4">
        <p className="text-center text-2xl font-bold text-slate-800 dark:text-slate-100">מה שייך למקום?</p>
        <button
          type="button"
          onClick={replay}
          disabled={busy}
      ...
      <div className="rounded-3xl border-4 border-teal-200 bg-teal-50/80 px-10 py-6 dark:border-teal-700 dark:bg-teal-950/30">
        <p className="text-center text-4xl font-bold">{round.place}</p>
      </div>
