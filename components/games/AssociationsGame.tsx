...
import { ASSOCIATION_ROUNDS, twemojiUrl } from "@/lib/game-data";
import { bumpGameModuleRound, recordGameAttempt } from "@/lib/game-progress";

export function AssociationsGame({ token }: { token: string }) {
  const [progress, setProgress] = useState(0);
  return (
    <GameShell token={token} progress={progress} onRoundComplete={() => bumpGameModuleRound(token, "associations")}>
      <AssociationsInner token={token} onProgress={setProgress} />
    </GameShell>
  );
}

function AssociationsInner({
  token,
  onProgress,
}: {
  token: string;
  ...
  const indices = useMemo(() => shuffle(ASSOCIATION_ROUNDS.map((_, i) => i)), []);
  const [cursor, setCursor] = useState(0);
  const [busy, setBusy] = useState(false);
  const [correctInWave, setCorrectInWave] = useState(0);

  const round = ASSOCIATION_ROUNDS[indices[cursor % indices.length]];
  ...
  function replay() {
    if (busy) return;
    resumeAudioContext();
    void speakHebrew(`מה מתאים ל־${round.cueLabel}?`);
  }

  function handlePick(file: string) {
    if (busy) return;
    if (file !== round.correct) {
      recordGameAttempt(token, "associations", "wrong", `«${round.cueLabel}» — בחירה לא נכונה`);
      playSoftBuzz();
      if (typeof navigator !== "undefined" && navigator.vibrate) navigator.vibrate(35);
      return;
    }
    setBusy(true);
    recordGameAttempt(token, "associations", "correct");
    playCorrectChime();
    triggerStars();
  ...
  return (
    <div className="flex flex-col items-center gap-8 px-2">
      <div className="flex flex-col items-center gap-4">
        <p className="text-center text-2xl font-bold text-slate-800 dark:text-slate-100">מה מתאים?</p>
        <button
          type="button"
          onClick={replay}
          disabled={busy}
      ...
      <div className="flex flex-col items-center gap-4 rounded-3xl border-4 border-violet-200 bg-violet-50/80 px-6 py-6 dark:border-violet-700 dark:bg-violet-950/30">
        <p className="text-3xl font-bold">{round.cueLabel}</p>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={twemojiUrl(round.cueFile)}
          alt=""
          width={120}
          height={120}
